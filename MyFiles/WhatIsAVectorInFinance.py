...
class Scene_02(Scene):
    ...
    def intro(self):
        text = TextMobject("Did you know?")

        self.play(Write(text))
        self.wait()
        # Did you know that linear algebra has a very nice application on finance?
        # When we learn linear algebra, the first topic of the course is usually a vector.
        self.play(FadeOut(text))

        vector = Vector([1, 2], color=YELLOW).shift(5*LEFT+DOWN)
        coordinates = Matrix([1, 2]).shift(5*RIGHT)
        symbol = TexMobject("\\vec{\\textbf{a}}")

        self.play(ShowCreation(vector))
        self.wait()
        # For physics students, it is an arrow.
        self.play(ShowCreation(coordinates))
        self.wait()
        # For computer science students, it is a list of numbers.
        self.play(ShowCreation(symbol))
        self.wait()
        # For math students, it is just a mathematical notation.
        self.remove(vector, coordinates, symbol)
        self.wait()
        return self

    def coin_tossing(self, s, **kwargs):
        c_1 = Circle(radius=0.5)
        s_1 = TextMobject(s[0])
        v_1 = VGroup(c_1, s_1)

        c_2 = Circle(radius=0.5)
        s_2 = TextMobject(s[1])
        v_2 = VGroup(c_2, s_2)
        v_2.rotate(TAU / 4, UP)
    ...
    def explanation(self):
        text = TextMobject(r'''
            \begin{itemize}
            \item debt securities (e.g., bonds)
            \item equity securities (e.g., common stocks) 
            \item derivatives (e.g., forwards and options)
            \end{itemize}
        ''')
